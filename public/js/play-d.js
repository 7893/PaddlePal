	var score_bar_enable = true;		// 是否开启水平滚动条
	var direction = 0;
	var pos = 0;
	...
	function name_refresh() {
		$.getJSON('playing?option=json', function(playing) {
			$.getJSON('toplay?option=json', function(toplay) {
				var maxTable = 0;
				var tbody = '';
				$.each(playing.array, function(i, obj) {
					maxTable = Math.max(maxTable, obj.tb);
				});
				$.each(toplay.array, function(i, obj) {
					maxTable = Math.max(maxTable, obj.tb);
				});
				for (var i = 1; i <= maxTable; i++) {
					...
					for (var j = 0; j < toplay.array.length; j++) {
						var obj = toplay.array[j];
						if (obj.tb == i && obj.nl.length > 0 && obj.nr.length > 0) {
							tdArray[6] = obj.tb;
							tdArray[7] = obj.tm;
							tdArray[8] = obj.gp + obj.ev;
							tdArray[9] = obj.nl;
							tdArray[10] = "—";
							tdArray[11] = obj.nr;
							find = true;
