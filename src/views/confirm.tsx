...
type Match = {
  id: number; match_order: number; time: string; event: string;
  p1: string; p2: string; score1: number; score2: number; games: string;
};
        ...
        {!isReferee ? (
          ...
          <>
            <div class="flex items-center justify-between mb-6">
              ...
              <button onclick="confirmAll()" class="px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-medium hover:from-emerald-400 hover:to-teal-400 shadow-lg shadow-emerald-500/25">
                全部确认
              </button>
            </div>
            <Card>
              <div class="space-y-3">
                {matches.map(m => (
                  <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl" data-id={m.id}>
                    <div class="flex-1">
                      <div class="text-sm text-slate-500 mb-1">{m.event} · {m.time}</div>
                      <div class="font-semibold text-lg">
                        <span class={m.score1 > m.score2 ? 'text-emerald-600' : 'text-slate-800'}>{m.p1}</span>
                        <span class="mx-3 text-slate-400">{m.score1} : {m.score2}</span>
                    ...
                    <div class="flex gap-3">
                      <button onclick={`confirmOne(${m.id})`} class="px-4 py-2 bg-emerald-100 text-emerald-700 rounded-xl text-sm font-medium hover:bg-emerald-200 transition-colors">
                        ✓ 确认
                      </button>
                      <a href={`/score/${m.id}`} class="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-sm font-medium hover:bg-slate-200 transition-colors">
                        修改
                      </a>
                    </div>
                  </div>
