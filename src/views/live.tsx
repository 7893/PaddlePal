...
export const LivePage: FC<{ playing: Match[]; upcoming: UpcomingMatch[] }> = ({ playing, upcoming }) => (
  <Layout title="实时比分">
    ...
    <PageWrapper>
      {/* Connection status */}
      <div id="connStatus" class="hidden fixed top-20 left-1/2 -translate-x-1/2 z-50 px-4 py-2 rounded-full text-sm font-medium shadow-lg"></div>
            ...
            <tbody class="divide-y divide-slate-100">
              {upcoming.map(m => (
                <tr class="hover:bg-slate-50 transition-colors">
                  ...
                  <td class="px-5 py-4"><span class="px-3 py-1 bg-slate-100 rounded-lg text-slate-700">{m.tb}号</span></td>
                  <td class="px-5 py-4">
                  ...
                  <td class="px-5 py-4 text-center">
                    <a href={`/score/${m.pid}`} class="px-3 py-1.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-lg text-xs font-medium transition-colors">记分</a>
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  return (
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 transition-all hover:shadow-lg">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="px-6">
          <div class="text-4xl font-bold text-slate-800">{gL} - {gR}</div>
