...
export const GroupStandings: FC<{ title: string; groups: { name: string; entries: Entry[] }[] }> = ({
  title,
  groups,
}) => (
                    ...
                    .map((e, i) => (
                      <tr class={`${i < 2 ? 'bg-emerald-50' : ''} hover:bg-slate-50 transition-colors`}>
                        <td class="py-3">
                          {i < 2 ? (
                            <span
                              class={`w-6 h-6 rounded-full inline-flex items-center justify-center text-white text-xs font-bold ${i === 0 ? 'bg-amber-400' : 'bg-slate-400'}`}
                            >
                              {i + 1}
                            </span>
                          ) : (
