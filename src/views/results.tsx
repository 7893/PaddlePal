...
type CrossGroup = {
  name: string;
  header: string[];
  rows: { player: [number, string, string]; cells: string[]; points: number; rank: number }[];
};
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {events.map((ev) => {
          ...
          return (
            <a
              href={`/results/${ev.key}`}
              class="block bg-white rounded-2xl p-6 border border-slate-200 hover:shadow-lg hover:border-slate-300 transition-all"
            >
              ...
              <div class="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                <div
                  class="bg-gradient-to-r from-emerald-400 to-teal-500 h-2 rounded-full"
                  style={`width:${pct}%`}
                ></div>
              </div>
            </a>
  ...
  <Layout title={title}>
    ...
    <PageWrapper>
      <div class="flex items-center gap-3 mb-6">
        <a href="/results" class="text-slate-400 hover:text-slate-600 transition-colors">
          ← 返回
        </a>
                    ...
                    {g.rows.map((r) => (
                      <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-5 py-3 w-14">
                          {r[0] === 1 ? (
                            <span class="w-8 h-8 rounded-full bg-amber-400 flex items-center justify-center text-white font-bold">
                              1
                            </span>
                          ) : r[0] === 2 ? (
                            <span class="w-8 h-8 rounded-full bg-slate-400 flex items-center justify-center text-white font-bold">
                              2
                            </span>
                          ) : r[0] === 3 ? (
                            <span class="w-8 h-8 rounded-full bg-amber-600 flex items-center justify-center text-white font-bold">
                              3
                            </span>
                          ) : (
                ...
                <table class="w-full text-sm">
                  <thead>
                    <tr class="bg-slate-50">
                      ...
                      {g.header.map((h) => (
                        <th class="px-3 py-2 text-center text-slate-500 font-semibold w-14">{h}</th>
                      ))}
                  ...
                  <tbody class="divide-y divide-slate-100">
                    {g.rows.map((r, i) => (
                      <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-3 py-2 font-semibold text-slate-800 sticky left-0 bg-white whitespace-nowrap">
                          {r.player[0]}. {r.player[1]}
                        </td>
                        ...
                        <td class="px-3 py-2 text-center">
                          {r.rank === 1 ? (
                            <span class="w-6 h-6 rounded-full bg-amber-400 inline-flex items-center justify-center text-white text-xs font-bold">
                              1
                            </span>
                          ) : r.rank === 2 ? (
                            <span class="w-6 h-6 rounded-full bg-slate-400 inline-flex items-center justify-center text-white text-xs font-bold">
                              2
                            </span>
                          ) : r.rank > 0 ? (
                            r.rank
