    ...
    <PageWrapper>
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex gap-2">
          ...
          <button
            onclick="clearDraw()"
            class="px-4 py-2.5 border border-slate-200 text-slate-600 rounded-xl text-sm font-medium hover:bg-slate-50 transition-colors"
          >
            清空
          </button>
          <button
            onclick="genMatches()"
            class="px-4 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl text-sm font-medium hover:from-emerald-600 hover:to-teal-600 transition-colors shadow-lg shadow-emerald-500/25"
          >
            生成对阵
          </button>
        </div>
          ...
          {unassigned.map((p) => (
            <div class="inline-flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-4 py-2 text-sm hover:border-emerald-300 transition-colors">
              ...
              <select
                ...
                class="ml-1 text-xs border border-slate-200 rounded-lg px-2 py-1 focus:ring-2 focus:ring-emerald-500"
              >
