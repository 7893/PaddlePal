    ...
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-900 px-4 relative overflow-hidden">
      ...
      <div class="w-full max-w-sm relative z-10">
        ...
        <form method="post" action="/login" class="bg-white/10 backdrop-blur-xl rounded-2xl border border-white/20 p-8">
          {error && (
            <div class="mb-4 p-3 bg-red-500/20 text-red-200 text-sm rounded-xl border border-red-500/30">{error}</div>
          )}
          <div class="mb-5">
            ...
            <input
              ...
              required
              autofocus
              class="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-white/40 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-shadow"
              placeholder="请输入用户名"
            />
          </div>
          <div class="mb-6">
          ...
          <button
            type="submit"
            class="w-full py-3.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-medium hover:from-emerald-400 hover:to-teal-400 transition-all shadow-lg shadow-emerald-500/30"
          >
            登录
          </button>
        </form>
