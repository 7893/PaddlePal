          ...
          <tbody class="divide-y divide-gray-100">
            {events.map(e => (
              <tr class="hover:bg-gray-50">
                ...
                <td class="py-2"><button onclick={`del('event',${e.id})`} class="text-red-500 text-xs hover:underline">删除</button> <a href={`/admin/draw/${e.id}`} class="text-pp-600 text-xs hover:underline ml-2">抽签</a>{e.stage === 'knockout' && <a href={`/bracket/${e.id}`} class="text-blue-600 text-xs hover:underline ml-2">对阵图</a>}{e.type === 'XT' && <a href={`/team/${e.id}`} class="text-blue-600 text-xs hover:underline ml-2">团体</a>}</td>
              </tr>
            ))}
          </tbody>
        </table>
