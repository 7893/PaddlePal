      ...
      <PageWrapper>
        <div class="max-w-2xl mx-auto">
            ...
            <div class="relative">
              ...
              {match.status === 'finished' && (
                <div class="text-center mt-6">
                  <span
                    class={`px-4 py-1.5 rounded-full text-sm font-medium ${match.confirmed ? 'bg-emerald-500/30 text-emerald-300' : 'bg-amber-500/30 text-amber-300'}`}
                  >
          ...
          {games.length > 0 && (
            <Card title="局分详情" class="mb-6">
              <div class="grid grid-cols-7 gap-3 text-center">
                {games.map((g) => (
                  <div
                    class={`p-3 rounded-xl ${g.s1 > g.s2 ? 'bg-emerald-100 border-2 border-emerald-300' : g.s2 > g.s1 ? 'bg-red-100 border-2 border-red-300' : 'bg-slate-100'}`}
                  >
          ...
          <div class="mt-6 text-center">
            <a href="/live" class="text-emerald-600 hover:text-emerald-700 font-medium">
              ← 返回实时比分
            </a>
          </div>
        </div>
