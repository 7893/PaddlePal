    ...
    <div class="max-w-xl mx-auto px-6 py-8">
      ...
      <div id="quickScore" class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 mb-5">
        ...
        <div class="flex gap-4">
          <button
            onclick="addPoint(0)"
            class="flex-1 py-10 rounded-2xl bg-gradient-to-br from-blue-50 to-blue-100 hover:from-blue-100 hover:to-blue-200 active:from-blue-200 active:to-blue-300 transition text-blue-700 font-bold text-3xl touch-manipulation shadow-sm"
          >
            <span id="pointL">0</span>
          </button>
          <button
            onclick="addPoint(1)"
            class="flex-1 py-10 rounded-2xl bg-gradient-to-br from-red-50 to-red-100 hover:from-red-100 hover:to-red-200 active:from-red-200 active:to-red-300 transition text-red-700 font-bold text-3xl touch-manipulation shadow-sm"
          >
        ...
        <div class="flex gap-3 mt-4">
          <button
            onclick="undoPoint()"
            class="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-500 text-sm hover:bg-slate-50 transition-colors"
          >
            ↩ 撤销
          </button>
          <button
      ...
      <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 mb-5">
        ...
        <div class="flex gap-3">
          ...
          <button
            ...
            class="px-6 py-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-medium hover:from-emerald-400 hover:to-teal-400 transition-all shadow-lg shadow-emerald-500/25"
          >
            解析
          </button>
        </div>
      ...
      <div class="space-y-3">
        ...
        <div class="flex gap-3">
          <button
            ...
            class="flex-1 py-3.5 rounded-xl bg-amber-500 text-white font-medium hover:bg-amber-600 active:bg-amber-700 transition-colors shadow-lg shadow-amber-500/25 touch-manipulation"
          >
            ▶ 开始比赛
          </button>
          <button
            onclick="saveScore()"
            class="flex-1 py-3.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-medium hover:from-emerald-400 hover:to-teal-400 transition-all shadow-lg shadow-emerald-500/25 touch-manipulation"
          >
            ✓ 保存成绩
          </button>
        </div>
        <div class="flex gap-3">
          <button
            onclick="walkover(1)"
            class="flex-1 py-2.5 rounded-xl border border-red-200 text-red-600 text-sm hover:bg-red-50 transition-colors"
          >
            左弃权
          </button>
          <button
        ...
        <div class="flex gap-3">
          <a
            href={`/score/${m.pid - 1}`}
            class="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-sm text-center hover:bg-slate-50 transition-colors"
          >
            ← 上一场
          </a>
          <a
...
export const ScoreNotFound: FC<{ pid: string }> = ({ pid }) => (
  <Layout title="未找到">
    ...
    <div class="max-w-xl mx-auto px-6 py-20 text-center">
      ...
      <a href="/schedule" class="text-emerald-600 hover:text-emerald-700 font-medium">
        ← 返回赛程
      </a>
    </div>
    <Footer />
