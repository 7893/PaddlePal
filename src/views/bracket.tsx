...
export const BracketPage: FC<{ title: string; rounds: BMatch[][]; maxRound: number; eventKey?: string }> = ({
  title,
  rounds,
  maxRound,
  eventKey,
            ...
            {rounds.map((ms, ri) => {
              ...
              return (
                <div
                  ...
                  style={`padding-top:${paddingTop}px; gap:${gap - matchHeight}px`}
                >
                  ...
                  {ms.map((m, _mi) => (
                    <div class="relative">
                      {/* Connector lines */}
                      {ri > 0 && <div class="absolute -left-4 top-1/2 w-4 h-px bg-slate-200" />}
                      {ri < maxRound - 1 && <div class="absolute -right-4 top-1/2 w-4 h-px bg-slate-200" />}
                      {/* Match card */}
                      <div
                        class={`w-52 rounded-xl overflow-hidden shadow-sm transition-all ${
                          m.status === 'playing'
                        ...
                        <a href={m.pid ? `/score/${m.pid}` : '#'} class="block">
                          ...
                          >
                            <div class="flex items-center gap-2 min-w-0">
                              ...
                              <span
                                class={`truncate ${m.winner === 1 ? 'font-bold text-emerald-700' : m.p1 ? 'text-slate-700' : 'text-slate-300 italic'}`}
                              >
                                {m.p1 || '待定'}
                              </span>
                            </div>
                            {m.result && (
                              <span class="text-xs font-mono text-slate-400 ml-2">{m.result.split(':')[0]}</span>
                            )}
                          </div>
                          ...
                          >
                            <div class="flex items-center gap-2 min-w-0">
                              ...
                              <span
                                class={`truncate ${m.winner === 2 ? 'font-bold text-emerald-700' : m.p2 ? 'text-slate-700' : 'text-slate-300 italic'}`}
                              >
                                {m.p2 || '待定'}
                              </span>
                            </div>
                            {m.result && (
                              <span class="text-xs font-mono text-slate-400 ml-2">{m.result.split(':')[1]}</span>
                            )}
                          </div>
                        </a>
                      </div>
