            ...
            <tbody class="divide-y divide-slate-100">
              {matches.map(m => (
                <tr class="hover:bg-slate-50 transition-colors">
                  <td class="px-5 py-4">
                    <a href={`/score/${m.pid}`} class="text-emerald-600 hover:text-emerald-700 font-medium">{m.pid}</a>
                  </td>
                  ...
                  <td class="px-5 py-4">
                    <span class={`font-semibold ${m.result && parseInt(m.result) > parseInt(m.result.split(':')[1] || '0') ? 'text-emerald-600' : 'text-slate-800'}`}>
                      {m.player1 || '待定'}
                    </span>
                    <span class="text-slate-400 mx-2">vs</span>
                    <span class={`font-semibold ${m.result && parseInt(m.result.split(':')[1] || '0') > parseInt(m.result) ? 'text-emerald-600' : 'text-slate-800'}`}>
                      {m.player2 || '待定'}
                    </span>
                  </td>
