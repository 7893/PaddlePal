      ...
      <Card title="➕ 新增队伍" class="mt-4">
        <form id="f" class="flex gap-3">
          <input
            name="name"
            placeholder="队伍全称"
            class="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500"
            required
          />
          <input
            name="short_name"
            placeholder="简称"
            class="w-28 border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500"
            required
          />
          <button
            type="submit"
            class="px-6 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-medium hover:from-emerald-600 hover:to-teal-600"
          >
            添加
          </button>
        </form>
    ...
    <PageWrapper>
      ...
      <Card title="➕ 新增选手" class="mt-4">
        <form id="f" class="flex gap-3 flex-wrap">
          <input
            name="name"
            placeholder="姓名"
            class="flex-1 min-w-[120px] border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500"
            required
          />
          <select
            name="gender"
            class="w-20 border border-slate-200 rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-emerald-500"
          >
          ...
          <select
            name="team_id"
            class="w-36 border border-slate-200 rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-emerald-500"
          >
      ...
      <Card title="📋 批量导入" class="mt-4">
        <textarea
          id="bulk"
          rows={4}
          placeholder="每行一个：姓名,性别(M/W),队伍名"
          class="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm mb-3 focus:ring-2 focus:ring-emerald-500"
        ></textarea>
        <button
          onclick="bulkImport()"
      ...
      <Card title="➕ 发布公告" class="mt-4">
        <form id="f" class="space-y-3">
          ...
          <textarea
            ...
            placeholder="内容"
            rows={3}
            class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-emerald-500"
            required
          ></textarea>
          <button
            type="submit"
