    ...
    <PageWrapper>
      ...
      <Card title={`👥 队伍列表 (${teams.length})`}>
        <table class="w-full text-sm">
          <thead><tr class="text-left text-slate-500 border-b border-slate-200"><th class="pb-3">队名</th><th class="pb-3">简称</th><th class="pb-3">人数</th><th class="pb-3"></th></tr></thead>
          <tbody class="divide-y divide-slate-100">
            {teams.map(t => (
              <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="py-3"><button onclick={`del(${t.id})`} class="text-red-500 text-xs hover:underline">删除</button></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Card title="➕ 新增队伍" class="mt-4">
        <form id="f" class="flex gap-3">
          <input name="name" placeholder="队伍全称" class="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500" required />
          <input name="short_name" placeholder="简称" class="w-28 border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500" required />
          <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-medium hover:from-emerald-600 hover:to-teal-600">添加</button>
        </form>
      </Card>
    </PageWrapper>
    <Footer />
  ...
  <Layout title="选手管理">
    ...
    <PageWrapper>
      ...
      <Card title={`🏓 选手列表 (${players.length})`}>
        <table class="w-full text-sm">
          <thead><tr class="text-left text-slate-500 border-b border-slate-200"><th class="pb-3">姓名</th><th class="pb-3">性别</th><th class="pb-3">队伍</th><th class="pb-3">积分</th><th class="pb-3"></th></tr></thead>
          <tbody class="divide-y divide-slate-100">
            {players.map(p => (
              <tr class="hover:bg-slate-50 transition-colors">
                ...
                <td class="py-3"><button onclick={`del(${p.id})`} class="text-red-500 text-xs hover:underline">删除</button></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <Card title="➕ 新增选手" class="mt-4">
        <form id="f" class="flex gap-3 flex-wrap">
          <input name="name" placeholder="姓名" class="flex-1 min-w-[120px] border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500" required />
          <select name="gender" class="w-20 border border-slate-200 rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-emerald-500">
            <option value="M">男</option><option value="W">女</option><option value="X">混</option>
          </select>
          <select name="team_id" class="w-36 border border-slate-200 rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-emerald-500">
      ...
      <Card title="📋 批量导入" class="mt-4">
        <textarea id="bulk" rows={4} placeholder="每行一个：姓名,性别(M/W),队伍名" class="w-full border border-slate-200 rounded-xl px-4 py-3 text-sm mb-3 focus:ring-2 focus:ring-emerald-500"></textarea>
        <button onclick="bulkImport()" class="px-6 py-2.5 bg-blue-500 text-white rounded-xl font-medium hover:bg-blue-600 text-sm">文本导入</button>
        <div class="mt-4 pt-4 border-t border-slate-100">
          ...
          <button onclick="xlImport()" class="ml-2 px-4 py-2 bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 text-sm">导入</button>
        </div>
      </Card>
    </PageWrapper>
    <Footer />
...
export const NoticesEditPage: FC<{ notices: Notice[] }> = ({ notices }) => (
  <Layout title="公告管理">
    ...
    <PageWrapper>
      ...
      <Card title="📢 公告列表">
        {notices.length === 0 ? <div class="text-slate-400 text-center py-8">暂无公告</div> : (
          <div class="space-y-3">
            {notices.map(n => (
              <div class="border border-slate-200 rounded-xl p-4 flex justify-between items-start hover:bg-slate-50 transition-colors">
                <div><div class="font-medium text-slate-800">{n.title || '(无标题)'}</div><div class="text-sm text-slate-500 mt-1">{n.content}</div><div class="text-xs text-slate-400 mt-2">{n.created_at}</div></div>
      ...
      <Card title="➕ 发布公告" class="mt-4">
        <form id="f" class="space-y-3">
          <input name="title" placeholder="标题" class="w-full border border-slate-200 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-emerald-500" />
          <textarea name="content" placeholder="内容" rows={3} class="w-full border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-emerald-500" required></textarea>
          <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-medium hover:from-emerald-600 hover:to-teal-600">发布</button>
        </form>
      </Card>
    </PageWrapper>
    <Footer />
