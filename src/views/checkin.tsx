...
type Match = {
  ...
  p1: string; p2: string; checkin1: number; checkin2: number;
};
              ...
              {matches.map(m => (
                <div class="p-5 bg-slate-50 rounded-xl" data-id={m.id}>
                  <div class="flex justify-between items-center mb-4">
                    ...
                    <span class="px-3 py-1 bg-slate-200 text-slate-600 rounded-lg text-sm">{m.table_no}号台 · {m.event}</span>
                  </div>
                  <div class="grid grid-cols-2 gap-4">
                    <div class={`p-4 rounded-xl border-2 cursor-pointer transition-all ${m.checkin1 ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-white hover:border-emerald-300 hover:shadow-md'}`} onclick={`checkin(${m.id}, 1)`}>
                      <div class="font-semibold text-slate-800">{m.p1}</div>
                      <div class="text-sm mt-2">{m.checkin1 ? <span class="text-emerald-600">✅ 已检录</span> : <span class="text-slate-400">点击检录</span>}</div>
                    </div>
                    <div class={`p-4 rounded-xl border-2 cursor-pointer transition-all ${m.checkin2 ? 'border-emerald-500 bg-emerald-50' : 'border-slate-200 bg-white hover:border-emerald-300 hover:shadow-md'}`} onclick={`checkin(${m.id}, 2)`}>
                      <div class="font-semibold text-slate-800">{m.p2}</div>
                      <div class="text-sm mt-2">{m.checkin2 ? <span class="text-emerald-600">✅ 已检录</span> : <span class="text-slate-400">点击检录</span>}</div>
                    </div>
                  </div>
                </div>
              ))}
