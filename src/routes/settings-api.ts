...
app.post('/api/settings', async (c) => {
  ...
  if (updates.length > 0) {
    await db
      .prepare(`UPDATE tournaments SET ${updates.join(', ')} WHERE id = 1`)
      .bind(...params)
      .run();
  }
...
app.get('/api/backup', async (c) => {
  ...
  c.header(
    'Content-Disposition',
    `attachment; filename="paddlepal-backup-${new Date().toISOString().slice(0, 10)}.json"`
  );
...
app.post('/api/restore', async (c) => {
  ...
  for (const t of teams || []) {
    await db
      .prepare('INSERT INTO teams (id, tournament_id, name, short_name) VALUES (?, ?, ?, ?)')
      .bind(t.id, t.tournament_id, t.name, t.short_name)
      .run();
  }

  for (const p of players || []) {
    await db
      .prepare('INSERT INTO players (id, tournament_id, team_id, name, gender, rating) VALUES (?, ?, ?, ?, ?, ?)')
      .bind(p.id, p.tournament_id, p.team_id, p.name, p.gender, p.rating)
      .run();
  }

  for (const e of events || []) {
    await db
      .prepare('INSERT INTO events (id, tournament_id, key, title, event_type, best_of) VALUES (?, ?, ?, ?, ?, ?)')
      .bind(e.id, e.tournament_id, e.key, e.title, e.event_type, e.best_of)
      .run();
  }

  return c.json({
    success: true,
    restored: { teams: teams?.length, players: players?.length, events: events?.length },
  });
});
