...
adminApi.post('/api/admin/match/save', async (c) => {
  const { match_id, scores } = await c.req.json<{
    match_id: number;
    scores: { game: number; left: number; right: number }[];
  }>();
  const db = c.env.DB;
...
adminApi.post('/api/admin/match/walkover', async (c) => {
  ...
  const db = c.env.DB;
  const ev = await db
    .prepare('SELECT COALESCE(e.best_of,5) as bo FROM matches m JOIN events e ON m.event_id=e.id WHERE m.id=?')
    .bind(match_id)
    .first();
  ...
  if (walkover_side === 1) {
    result = `W-0:${wn}`;
    ws = 2;
  } else if (walkover_side === 2) {
    result = `${wn}:W-0`;
    ws = 1;
  } else if (walkover_side === 3) {
    result = '双弃权';
    ws = 0;
...
adminApi.get('/api/admin/players', async (c) => {
  ...
  if (id) {
    const p = await c.env.DB.prepare('SELECT id, name, gender, COALESCE(team_id,0) as team_id FROM players WHERE id=?')
      .bind(id)
      .first();
...
adminApi.post('/api/admin/players', async (c) => {
  const { id, name, gender, team_id } = await c.req.json<{
    id?: number;
    name: string;
    gender: string;
    team_id?: number;
  ...
  if (!id)
    await c.env.DB.prepare('INSERT INTO players (tournament_id, name, gender, team_id) VALUES (1,?,?,?)')
      .bind(name, gender, team_id || 0)
      .run();
  else
    await c.env.DB.prepare('UPDATE players SET name=?, gender=?, team_id=? WHERE id=?')
      .bind(name, gender, team_id || 0, id)
      .run();
  return c.json({ success: true });
});
...
adminApi.get('/api/admin/teams', async (c) => {
  const id = c.req.query('id');
  if (id) {
    const t = await c.env.DB.prepare("SELECT id, name, COALESCE(short_name,'') as short_name FROM teams WHERE id=?")
      .bind(id)
      .first();
...
adminApi.post('/api/admin/teams', async (c) => {
  ...
  if (!id)
    await c.env.DB.prepare('INSERT INTO teams (tournament_id, name, short_name) VALUES (1,?,?)')
      .bind(name, short_name)
      .run();
...
adminApi.post('/api/admin/tournament', async (c) => {
  ...
  await c.env.DB.prepare('UPDATE tournaments SET info=?, venue=?, start_date=? WHERE id=1')
    .bind(info, venue, start_date)
    .run();
  return c.json({ success: true });
});
...
adminApi.get('/api/admin/events', async (c) => {
  ...
  if (id) {
    const e = await c.env.DB.prepare(
      "SELECT id, title, event_type as type, COALESCE(stage,'loop') as stage, groups, best_of FROM events WHERE id=?"
    )
      .bind(id)
      .first();
  ...
  const { results } = await c.env.DB.prepare(
    "SELECT id, title, event_type as type, COALESCE(stage,'loop') as stage, groups, best_of FROM events WHERE tournament_id=1"
  ).all();
...
adminApi.post('/api/admin/events', async (c) => {
  const {
    ...
    groups,
    best_of,
  } = await c.req.json<{ id?: number; title: string; type: string; stage?: string; groups: number; best_of: number }>();
  ...
  if (!id) {
    ...
    const r = await db
      .prepare(
        'INSERT INTO events (tournament_id, key, event_type, title, stage, groups, best_of) VALUES (1,?,?,?,?,?,?)'
      )
    ...
    await db
      .prepare('UPDATE events SET title=?, event_type=?, stage=?, groups=?, best_of=? WHERE id=?')
      .bind(title, type, stage, groups, best_of, id)
      .run();
  }
  return c.json({ success: true });
});
...
adminApi.delete('/api/admin/events', async (c) => {
  ...
  await c.env.DB.batch([
    ...
    c.env.DB.prepare('DELETE FROM group_entries WHERE group_id IN (SELECT id FROM group_tables WHERE event_id=?)').bind(
      id
    ),
...
adminApi.post('/api/admin/draw/assign', async (c) => {
  const { event_id, player_id, group_index } = await c.req.json<{
    event_id: number;
    player_id: number;
    group_index: number;
  }>();
  const db = c.env.DB;
  const g = await db
    .prepare('SELECT id FROM group_tables WHERE event_id=? AND group_index=?')
    .bind(event_id, group_index)
    .first();
  const mp = await db
    .prepare('SELECT COALESCE(MAX(position),0) as p FROM group_entries WHERE group_id=?')
    .bind(g!.id)
    .first();
  await db
    .prepare('INSERT INTO group_entries (group_id, player_id, position) VALUES (?,?,?)')
    .bind(g!.id, player_id, (mp!.p as number) + 1)
    .run();
  return c.json({ success: true });
});
...
adminApi.post('/api/admin/draw/clear', async (c) => {
  ...
  await c.env.DB.prepare('DELETE FROM group_entries WHERE group_id IN (SELECT id FROM group_tables WHERE event_id=?)')
    .bind(event_id)
    .run();
  return c.json({ success: true });
});
...
adminApi.post('/api/admin/draw/auto', async (c) => {
  ...
  await db
    ...
    .bind(event_id)
    .run();
  const { results: gs } = await db
    .prepare('SELECT id FROM group_tables WHERE event_id=? ORDER BY group_index')
    .bind(event_id)
    .all();
...
adminApi.post('/api/admin/notice', async (c) => {
  ...
  if (!id)
    await c.env.DB.prepare(
      "INSERT INTO notices (tournament_id, title, content, created_at) VALUES (1,?,?,datetime('now'))"
    )
      .bind(title, content)
      .run();
...
adminApi.get('/api/admin/export/schedule', async (c) => {
  ...
  for (const r of results)
    csv += `${r.pid},${r.date || ''},${r.time || ''},${r.table_no},${r.player1},${r.player2},${r.event},${r.result || ''},${r.status}\n`;
  return new Response(csv, {
    headers: { 'Content-Type': 'text/csv; charset=utf-8', 'Content-Disposition': 'attachment; filename=schedule.csv' },
  });
});

adminApi.get('/api/admin/export/players', async (c) => {
  const { results } = await c.env.DB.prepare(
    `SELECT p.name, p.gender, COALESCE(t.name,'') as team, COALESCE(p.rating,0) as rating
    FROM players p LEFT JOIN teams t ON p.team_id=t.id WHERE p.tournament_id=1 ORDER BY p.id`
  ).all();
  const bom = '\uFEFF';
  ...
  return new Response(csv, {
    headers: { 'Content-Type': 'text/csv; charset=utf-8', 'Content-Disposition': 'attachment; filename=players.csv' },
  });
});
...
adminApi.get('/api/admin/backup', async (c) => {
  ...
  return new Response(JSON.stringify(data, null, 2), {
    headers: {
      ...
      'Content-Disposition': `attachment; filename=paddlepal-backup-${Date.now()}.json`,
    },
  });
});
