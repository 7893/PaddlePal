...
app.post('/api/rating/calc/:matchId', async (c) => {
  ...
  await db.batch([
    db
      .prepare(
        `INSERT INTO ratings (tournament_id, player_id, event_id, match_id, rating_before, rating_after, rating_change)
      VALUES (1, ?, ?, ?, ?, ?, ?)`
      )
      .bind(winnerId, match.event_id, matchId, winnerRating, winnerRating + winChange, winChange),
    db
      .prepare(
        `INSERT INTO ratings (tournament_id, player_id, event_id, match_id, rating_before, rating_after, rating_change)
      VALUES (1, ?, ?, ?, ?, ?, ?)`
      )
      .bind(loserId, match.event_id, matchId, loserRating, loserRating + loseChange, loseChange),
  ...
  for (const m of matches) {
    ...
    await db.batch([
      db
        ...
        .bind(winnerId, event.id, m.id, winnerRating, winnerRating + winChange, winChange),
      db
        .prepare(
          `INSERT INTO ratings (tournament_id, player_id, event_id, match_id, rating_before, rating_after, rating_change)
        VALUES (1, ?, ?, ?, ?, ?, ?)`
        )
        .bind(loserId, event.id, m.id, loserRating, loserRating + loseChange, loseChange),
      db.prepare('UPDATE players SET rating = rating + ? WHERE id = ?').bind(winChange, winnerId),
      db.prepare('UPDATE players SET rating = rating + ? WHERE id = ?').bind(loseChange, loserId),
    ]);
    calculated++;
