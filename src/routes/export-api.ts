...
app.get('/api/export/program', async (c) => {
  ...
  const tournament = await db.prepare('SELECT title, venue, start_date, end_date FROM tournaments WHERE id = 1').first();
  const { results: events } = await db.prepare('SELECT id, title, type, best_of FROM events WHERE tournament_id = 1').all();
...
app.get('/api/export/csv/:type', async (c) => {
  ...
  if (type === 'players') {
    ...
    `).all();
    csv = '项目,轮次,选手A,选手B,比分A,比分B,局分\n' + results.map(r => `${r.event},${r.round},${r.player1},${r.player2},${r.score1},${r.score2},${r.games || ''}`).join('\n');
  }
