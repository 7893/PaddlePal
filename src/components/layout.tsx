...
export const Layout: FC<{ title: string; children: Child }> = ({ title, children }) => (
  <html lang="zh-CN">
    ...
    <body class="bg-slate-50 min-h-screen">{children}</body>
  </html>
);
            ...
            <div class="flex items-center justify-between h-16">
              ...
              <div class="hidden md:flex items-center gap-1">
                {menu.map((item) => (
                  <a
                    href={item.href}
                    class={`px-4 py-2 text-sm rounded-lg transition-colors ${isActive(item.href) ? 'bg-white/20 text-white font-medium' : 'text-white/70 hover:text-white hover:bg-white/10'}`}
                  >
                    {item.label}
                  </a>
                ))}
                {adminMenu.map((item) => (
                  <a
                    href={item.href}
                    class={`admin-only px-4 py-2 text-sm rounded-lg transition-colors ${isActive(item.href) ? 'bg-emerald-500/30 text-emerald-300 font-medium' : 'text-emerald-400/70 hover:text-emerald-300 hover:bg-white/10'}`}
                    style="display:none"
                  >
                    {item.label}
                  </a>
              ...
              <div class="flex items-center gap-3">
                <div id="user-info" class="hidden items-center gap-3">
                  ...
                  <a href="/logout" class="text-sm text-white/50 hover:text-red-400">
                    退出
                  </a>
                </div>
                <a
                  href="/login"
                  id="login-btn"
                  class="px-4 py-2 bg-white/10 hover:bg-white/20 text-white text-sm rounded-lg border border-white/20"
                >
                  登录
                </a>
                <button
                  class="md:hidden p-2 text-white/70"
        ...
        <div id="mobile-menu" class="hidden md:hidden border-t border-white/10">
          <div class="px-4 py-3 space-y-1">
            {menu.map((item) => (
              <a
                href={item.href}
                class={`block px-4 py-2 rounded-lg text-sm ${isActive(item.href) ? 'bg-white/20 text-white' : 'text-white/70'}`}
              >
                {item.label}
              </a>
            ))}
            {adminMenu.map((item) => (
              <a
                href={item.href}
                class={`admin-only block px-4 py-2 rounded-lg text-sm ${isActive(item.href) ? 'bg-emerald-500/30 text-emerald-300' : 'text-emerald-400/70'}`}
                style="display:none"
              >
                {item.label}
              </a>
...
export const Card: FC<{ title?: string; children: Child; class?: string; hover?: boolean }> = (props) => (
  <div
    class={`bg-white rounded-2xl shadow-sm border border-slate-200/60 ${props.hover !== false ? 'card-hover' : ''} ${props.class || ''}`}
  >
...
export const Th: FC<{ children: Child; class?: string }> = ({ children, class: cls }) => (
  <th
    class={`px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider bg-slate-50 ${cls || ''}`}
  >
    {children}
  </th>
);
    ...
    <div class="max-w-7xl mx-auto px-6">
      <div class="flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div class="flex gap-6 text-sm">
          ...
          <a href="/qr" class="hover:text-white">
            扫码入口
          </a>
        </div>
      </div>
...
export const EmptyState: FC<{ icon?: string; message?: string; title?: string; description?: string }> = ({
  icon = '📭',
  message,
  title,
  description,
